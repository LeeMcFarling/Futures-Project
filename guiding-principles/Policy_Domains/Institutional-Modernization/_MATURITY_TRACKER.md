# Domain Maturity Tracker: Institutional-Modernization

**Last Updated:** 2026-02-08
**Overall Domain Phase:** 3
**Domain Owner:** TBD

---

## Phase Model Reference

- **Phase 0** – Problem Framing
- **Phase 1** – Strategic Constraints
- **Phase 2** – Structured Exploration
- **Phase 3** – Outcomes & Decision Rules
- **Phase 4** – Sequencing & Execution Design
- **Phase 5** – External Validation
- **Phase 6** – Messaging & Rebuttal

---

## Domain Overview

**Purpose:**
Modernize democratic institutions through reusable execution instruments that translate democratically authorized objectives into operational reality under bounded authority, cost, and time constraints.

**Key Questions:**
- How do we enable adaptive governance without technocracy?
- What execution structures solve delivery failure without expanding state power?
- How do we deploy public capital deliberately and reversibly?

**Success Criteria:**
- Execution Corps instrument specification deployed across domains
- Public Capital Authority operational with tax-expenditure conversion underway
- Government capable of pilot-first implementation with planned exit

---

## Subdomain Status Matrix

| Subdomain | Phase | File Count | Word Count | Key Gaps | Dependencies | Next Action | Target Date |
|-----------|-------|------------|------------|----------|--------------|-------------|-------------|
| Guiding_Principles | 3 | 1 | ~1,200 | None | None | Complete | — |
| Execution_Corps_Spec | 3 | 1 | ~2,100 | Domain instantiations | None | Add domain-specific deployments | TBD |
| Industry_Interface | 3 | 1 | ~1,300 | Sector examples | Execution_Corps_Spec | Add case studies | TBD |
| Public_Capital_Authority | 3 | 1 | ~1,500 | Portfolio definitions | Budget.Fiscal_Policy | Define initial portfolios | TBD |
| Regulatory_Compression | 0 | 0 | 0 | **Not started** | None | Create initial file | TBD |
| Process_Redesign | 0 | 0 | 0 | **Not started** | None | Create initial file | TBD |

---

## Dependency Map

### Hard Dependencies (Blocking)
| This Domain Requires | From Domain | Status | Risk Level |
|---------------------|-------------|--------|------------|
| Fiscal policy framework | Economic_Prosperity.Budget | Phase 2 | Medium |

### Soft Dependencies (Coordination)
| This Domain Benefits From | From Domain | Status | Notes |
|--------------------------|-------------|--------|-------|
| Digital identity infrastructure | Technology_and_Data | Phase 2 | Citizen feedback systems |
| Democratic legitimacy framework | Democratic_Integrity | Phase 2 | Oversight mechanisms |

### Provides To (Downstream Impact)
| Other Domain | What We Provide | Criticality |
|--------------|-----------------|-------------|
| All domains | Execution Corps instrument | High |
| Economic_Prosperity | Public Capital Authority | High |
| Healthcare | Execution pathway for HETA | High |
| Public_Infrastructure | Capital deployment model | High |

---

## Gap Analysis

### Tier 1: Critical Gaps (Missing Foundation)
- [x] **Execution Corps Specification** – Core instrument defined ✅ COMPLETED
- [x] **Public Capital Authority** – Standing capital corps defined ✅ COMPLETED

### Tier 2: Structural Gaps (Incomplete Coverage)
- [ ] **Regulatory Compression** – Permitting reform, process streamlining – Target: Phase 2
- [ ] **Process Redesign** – Administrative burden reduction – Target: Phase 2
- [ ] **Domain Instantiations** – Sector-specific Execution Corps deployments – Target: Phase 3

### Tier 3: Enhancement Gaps (Nice-to-Have)
- [ ] **Case Studies** – Industry interface examples – Target: Phase 4
- [ ] **Metrics Framework** – Standardized execution measurement – Target: Phase 4

---

## Phase Advancement Checklist

### Requirements for Phase 4 (Sequencing & Execution)
- [x] Core instrument specification complete
- [x] Standing corps (PCA) defined
- [ ] Initial domain instantiations documented
- [ ] Portfolio definitions for PCA established
- [ ] Pilot jurisdiction criteria defined

---

## Recent Activity Log

| Date | Subdomain | Change | Phase Impact |
|------|-----------|--------|--------------|
| 2026-02-08 | All | Created maturity tracker | N/A |
| 2026-01-28 | Execution_Corps_Spec | YAML standardized, domain field added | N/A |
| 2026-01-28 | All | Domain created, files moved from other locations | 0 → 3 |

---

## Notes & Context

- Domain has 4 active policy files totaling ~6,100 words (excluding archives)
- 2 archive files preserved (old_* prefix) showing policy evolution
- Execution Corps is a cross-cutting instrument used by multiple domains
- Public Capital Authority is Phase 3 deployment-ready
- Strong conceptual coherence between guiding principles and specifications
- Industry Interface brief provides private-sector translation of core concepts
- Industry Interface document tagged with platform-regulation and firm-regulation for cross-domain linkage
